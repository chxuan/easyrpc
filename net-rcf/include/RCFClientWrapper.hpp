...
template<typename I_RCFMessageHandler>
RCFClientWrapper<I_RCFMessageHandler>::RCFClientWrapper(const std::string& ip, unsigned int port)
{
    m_impl.reset();
    if (m_impl == NULL)
    {
...
template<typename I_RCFMessageHandler>
RcfClientPtr RCFClientWrapper<I_RCFMessageHandler>::rcfClientObject() const
{
    assert(m_impl != NULL);
...
template<typename I_RCFMessageHandler>
void RCFClientWrapper<I_RCFMessageHandler>::setConnectTimeoutMs(long long time)
{
    assert(m_impl != NULL);
...
template<typename I_RCFMessageHandler>
void RCFClientWrapper<I_RCFMessageHandler>::setRemoteCallTimeoutMs(long long time)
{
    assert(m_impl != NULL);
